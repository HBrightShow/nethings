...
bool CRedisConn::Connect(const std::string &addr, int port, const std::string &pwd,int db) {
    addr_ = addr;
    port_ = port;
    pwd_ = pwd;
...
*/
int CRedisConn::ConnectAuth(const std::string &psw)
{
...
int CRedisConn::SelectDb(const int &db)
{
...
bool CRedisConn::IsConnect()
{
    return is_connect_;
}

RedisStatus CRedisConn::CheckErr(redisReply *reply)
{
    if(reply == NULL)
    {
        reply = pm_rr;
...
RedisStatus CRedisConn::ParseReplay(RedisResult &result,redisReply *reply)
{
    if(reply == NULL)
    {
        reply = pm_rr;
...
int CRedisConn::FreeRedisReply(redisReply *reply)
{
    if (reply)
    {
...
RedisStatus CRedisConn::ExecCmd(const std::string& cmd, RedisResult& res)
{
    pm_rr = (redisReply*)redisCommand(pm_rct, cmd.c_str());
